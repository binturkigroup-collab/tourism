...
const ContactShow: React.FC<{contact: Contact, agents: Block []}> = ({contact, agents}) => {
    const contactService = Container.get(ContactService);
    const blockService = Container.get(BlockService);
    const [agent, setAgent] = React.useState<number>(contact.agentId || -1);
    // =========================================================================================
    ...
    const deleteBlock = () => {
        ...
        contactService.destroy(contact.id, formData)
            .then(response => {
                setSnackbar(snackbarState =>
    ...
    const handleAgent = async (event:  React.ChangeEvent<Omit<HTMLInputElement, "value"> & {
        ...
        if (event.target.value > 0) {
            ...
            contactService.agent(formData, contact.id)
                .then(() => {
                    setSnackbar(snackbarState =>
                ...
                <div className="max-w-2xl mx-auto">
                    <div className="flex items-center justify-between mb-6">
                        <div className="flex justify-start items-center">
                            ...
                            <Link href="/admin/contact" className="gap-2">
                                Back to Contacts
                            </Link>
                        </div>
                        ...
                        <CardContent className="space-y-6">
                            ...
                            <div className="flex items-start gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-lg font-medium">{CustomerType[contact.type]}</p>
                                </div>
                            </div>
                            ...
                            {contact.action && (
                                <div className="flex items-start gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-base">{contact.action}</p>
                                    </div>
                                </div>
                            )}
